...
static size_t p_playerCrashDirection[4] = { 5, 5, 5, 5 };
...
bool MMS_updateMinigame0(bool* victory, const uint64_t time)
{
	...
	for(size_t ze = 0; ze < 4; ++ze)
	{
		...
		if(p_tableDirec[ze] != 5)
		{
			...
			p_playerCrashDirection[ze] = p_tableDirec[ze];
		}else{
			PLEX_RECT newSpace;
			...
			if(p_checkUnstuck(resSpace, newSpace, p_playerCrashSpace[ze], p_playerCrashDirection[ze], &p_space)) newOrigin = p_space.origin;

			for(size_t uh = 0; uh < p_enemiesUsed; ++uh)
			{
				MMS0_ENEMY* enemy = p_enemies + uh;

				if(p_checkUnstuck(resSpace, newSpace, p_playerCrashSpace[ze], p_playerCrashDirection[ze], &enemy->rect))
				{
		...
		if(!enemy->dead)
		{
			...
			for(size_t ze = 0; ze < 4; ++ze)
			{
				...
				if(PLEX_rectCollision(enemy->rect, tableSpace, emomX, emomY))
				{
					...
					switch(p_playerCrashDirection[ze])
					{
...
void MMS_runMinigame0(const size_t chapter0spriteTex, const size_t pamaleanaSpriteTex)
{
	...
	while(!victory && !PLEX_isTerminated())
	{
		...
		{
			PLEX_updatePlex(PLEX_LANGUAGE_TITLE);
